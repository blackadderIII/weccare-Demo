...
export default function CardsUncompleted() {
  ...
  const [uncompletedCardsData, setUncompletedCardsData] = useState([]);
  ...
  const fetchCards = () => {
    setTimeout(() => {
      ...
      setUncompletedCardsData(uncompletedCards);
      setIsLoading(false);
    }, 2500);
  };
      ...
      const filteredCards = carecards.filter((card) => {
        ...
        if (filter === "username") {
          ...
          const lastNameMatch = card.observerLastname
            ...
            .includes(search.toLowerCase());
          return (
            (firstNameMatch || lastNameMatch) && card.status === "Uncompleted"
          );
        }
  ...
  const exportCards = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("Uncompleted Cards");
      ...
      worksheet.columns = [
        ...
        {
          header: "Observer Designation",
          key: "observerDesignation",
          width: 20,
        },
      ...
      // Add rows for each Uncompleted card
      carecards
        .filter((card) => card.status === "Uncompleted")
        .forEach((card) => {
          worksheet.addRow({
            cardID: card.cardID,
      ...
      saveAs(
        blob,
        `Uncompleted_Cards_${new Date().toISOString().slice(0, 10)}.xlsx`
      );

      setIsExporting(false);
      successToast("Uncompleted Cards exported successfully");
    } catch (error) {
      setIsExporting(false);
    ...
    <>
      <section className="care-cards">
        <div className="row-01">
          <SearchComponent
            placeholder={"Search Uncompleted Cards..."}
            search={search}
            onSearch={(value) => {
              setSearch(value);
              checkIsEmpty(value);
          ...
          {isLoading ? (
            ...
            <table>
              ...
              {searchResult ? (
                ...
                <tbody>
                  {uncompletedCardsData.map((card, index) => {
                    // const pic = profilePictures.find(pic => pic.email === card.observerEmail);
                    return (
                      <tr onClick={() => viewCard(card.cardID)} key={index}>
                        <td>{card.cardID}</td>
