...
export default function CardsCompleted() {
    ...
    const [completedCardsData, setCompletedCardData] = useState([])
  ...
  const fetchCards = () =>{
    setTimeout(() => {
        ...
        setCompletedCardData(completedCards);
        setIsLoading(false);
      }, 2500);
  }
    // --------------------------------------------------------------------------------------------------------------
     ...
     const filteredCards = carecards.filter((card) => {
        ...
        if (filter === "username") {
          ...
          const lastNameMatch = card.observerLastname
            ?.toLowerCase()
            .includes(search.toLowerCase());
          return (firstNameMatch || lastNameMatch) && card.status === "Completed";
        }
        ...
        return (
          valueToSearch?.includes(search.toLowerCase()) && card.status === "Completed"
        );
      });
    ...
    const exportCards = async () => {
        ...
        try {
          ...
          const worksheet = workbook.addWorksheet('Completed Cards');
          ...
          // Add rows for each Completed card
          carecards.filter((card) => card.status === 'Completed')
            .forEach((card) => {
              worksheet.addRow({
                cardID: card.cardID,
          ...
          saveAs(blob, `Completed_Cards_${new Date().toISOString().slice(0, 10)}.xlsx`);
      
          setIsExporting(false);
          successToast('Completed Cards exported successfully');
        } catch (error) {
          setIsExporting(false);
            ...
            <section className='care-cards'>
                <div className='row-01'>
                    <SearchComponent
                        ...
                        search={search}
                        onSearch={value => { setSearch(value); checkIsEmpty(value) }}
                        ...
                        isCrashed ?
                            ...
                            isEmpty ?
                                ...
                                isSearhing ?
                                    <LoadingComponent /> :
                                    emptySearch ?
                                        <NotFoundIcon name={"Oops! Can't find that"} /> :
                                        <table>
                                            <thead>
                                                <tr>
                                                    <th></th>
                                            ...
                                            {searchResult ?
                                                <tbody>
                                                    {searchResult.map((card, index) => {
                                                        ...
                                                        return (
                                                            <tr onClick={() => viewCard(card.cardID, pic.profilepicture)} key={index}>
                                                                <td>{card.cardID}</td>
                                                                <td>
                                                                    <div className='user-info'>
                                                                    <img src={faker.image.urlPicsumPhotos()} alt="profile" />
                                                                        <div>
                                                                            <h4>{card.observerFirstname + ' ' + card.observerLastname}</h4>
                                                                            <a href={`mailto:${card.observerEmail}`}>{card.observerEmail}</a>
                                                                        </div>
                                                                    </div>
                                                                </td>
